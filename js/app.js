...
(function () {
    ...
    navLinks.forEach((link) => {
      link.addEventListener("click", function (e) {
        ...
        if (targetPage === "documentation") {
          window.open(
            "https://github.com/Kelll31/VulneraAI_light",
            "_blank",
            "noopener,noreferrer",
          );
          return;
  ...
  function initializeScrollEffects() {
    ...
    const animateElements = document.querySelectorAll(
      ".feature-card, .docs-section, .hero-content",
    );
  ...
  function createDynamicAIBackground() {
    ...
    class Particle {
      ...
      update() {
        ...
        // Пульсация
        this.size =
          this.baseSize + Math.sin(Date.now() * 0.002 + this.pulsePhase) * 0.4;
    ...
    function drawBackground() {
      ...
      baseGradient.addColorStop(
        0,
        `rgba(15, 23, 42, ${0.96 + Math.sin(time) * 0.04})`,
      );
      ...
      spot1.addColorStop(
        0,
        `rgba(59, 130, 246, ${0.06 + Math.sin(time * 1.2) * 0.02})`,
      );
      ...
      spot2.addColorStop(
        0,
        `rgba(16, 185, 129, ${0.04 + Math.sin(time * 1.8) * 0.02})`,
      );
